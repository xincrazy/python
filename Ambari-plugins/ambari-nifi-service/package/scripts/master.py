...
class Master(Script):
    def install(self, env):
        ...
        snapshot_package = 'http://10.128.7.241/hdf/nifi-0.5.1.1.1.2.0-32-bin.zip'
        ...
        if params.setup_prebuilt:
            ...
            if not os.path.exists(params.temp_file):
                Execute('wget ' + snapshot_package + ' -O ' + params.temp_file + ' -a ' + params.nifi_log_file,
                        user=params.nifi_user)
            Execute('unzip ' + params.temp_file + ' -d ' + params.nifi_install_dir + ' >> ' + params.nifi_log_file,
                    user=params.nifi_user)
            ...
            Execute(
                'cd ' + params.nifi_install_dir + '; git clone https://git-wip-us.apache.org/repos/asf/nifi.git ' + params.nifi_dir + ' >> ' + params.nifi_log_file)
            Execute('chown -R ' + params.nifi_user + ':' + params.nifi_group + ' ' + params.nifi_dir)

            Execute('cd ' + params.nifi_dir + '; mvn -T C2.0 clean install -DskipTests >> ' + params.nifi_log_file,
                    user=params.nifi_user)
    ...
    def configure(self, env, isInstall=False):
        ...
        File(format("{params.conf_dir}/nifi.properties"), content=properties_content, owner=params.nifi_user,
             group=params.nifi_group)  # , mode=0777)

        # write out zookeeper.properties
        zookeeper_properties_content = InlineTemplate(params.nifi_zookeeper_properties)
        File(format("{params.conf_dir}/zookeeper.properties"), content=zookeeper_properties_content, owner=params.nifi_user,
             group=params.nifi_group)
        ...
        if isInstall:
            ...
            File(format("{params.conf_dir}/flow.xml"), content=flow_content, owner=params.nifi_user,
                 group=params.nifi_group)
            Execute(format("cd {params.conf_dir}; mv flow.xml.gz flow_$(date +%d-%m-%Y).xml.gz ;"),
                    user=params.nifi_user, ignore_failures=True)
        ...
        File(format("{params.conf_dir}/bootstrap.conf"), content=bootstrap_content, owner=params.nifi_user,
             group=params.nifi_group)

        # write out logback.xml
        logback_content = InlineTemplate(params.nifi_logback_content)
        File(format("{params.conf_dir}/logback.xml"), content=logback_content, owner=params.nifi_user,
             group=params.nifi_group)
    ...
    def start(self, env):
        ...
        Execute(
            'export JAVA_HOME=' + params.jdk64_home + ';' + params.bin_dir + '/nifi.sh start >> ' + params.nifi_log_file,
            user=params.nifi_user)

        Execute(
            'cat ' + params.bin_dir + '/nifi.pid' + " | grep pid | sed 's/pid=\(\.*\)/\\1/' > " + status_params.nifi_pid_file)
    ...
    def set_conf_bin(self, env):
        ...
        if params.setup_prebuilt:
            ...
            params.conf_dir = \
                glob.glob(
                    params.nifi_install_dir + '/' + params.nifi_dirname + '/nifi-assembly/target/nifi-*/nifi-*/conf')[
                    0]
            params.bin_dir = \
                glob.glob(
                    params.nifi_install_dir + '/' + params.nifi_dirname + '/nifi-assembly/target/nifi-*/nifi-*/bin')[
                    0]
